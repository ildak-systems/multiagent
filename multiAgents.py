...
class ExpectimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        def maximizer(state, agentIndex, depth):
            ...
            for action in possibleActions:
                nextState = state.generateSuccessor(agentIndex, action)
                result = expectimax(nextState, nextAgent, depth + 1, action)
                if value < result[0]:
                    value = result[0]
                    # Replace the return value: chosenAction that was chosen
                    chosenAction = action
        ...
        def expectizer(state, agentIndex, depth):
            value = 0.0
            chosenAction = None
            ...
            possibleActions = state.getLegalActions(agentIndex)
            # Get the number of legal moves in this state
            numLegalMoves = len(possibleActions)
            for action in possibleActions:
                nextState = state.generateSuccessor(agentIndex, action)
                # uniform probability
                p = 1.0 / float(numLegalMoves)
                value += p * expectimax(nextState, nextAgent, depth + 1, action)[0]
                chosenAction = action
            return [value, chosenAction]

        def expectimax(state, agentIndex, currentDepth, action):
            ...
            if state.getNumAgents() * self.depth <= currentDepth:
                ...
                # at layer 1. I can't think of a more efficient method right now
                # Stick it with passing it every recursive return
                return [float(self.evaluationFunction(state)), action]
            ...
            if agentIndex == 0:
                ...
                return expectizer(state, agentIndex, currentDepth)
